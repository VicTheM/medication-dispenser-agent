...
Install deps:
    pip install opencv-python numpy httpx s3fs

Run locally:
    SERVER_TYPE=video PORT=5001 python backend_server.py
    SERVER_TYPE=audio PORT=5002 python backend_server.py
"""
...
def video_server(port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((HOST, port))
        s.listen(1)
        log("VIDEO", f"Listening on {HOST}:{port}")
        while True:
            conn, addr = s.accept()
            with conn:
                try:
...
def audio_server(port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        s.bind((HOST, port))
        s.listen(1)
        log("AUDIO", f"Listening on {HOST}:{port}")
        while True:
            conn, addr = s.accept()
            with conn:
                try:
...
def main():
    ...
    server_type = os.getenv("SERVER_TYPE", "").strip().lower()

    if server_type == "video":
        log("MAIN", f"Starting exclusively VIDEO server on port {port}")
        video_server(port)
