...
def enrol_patient(
    ...
    if payload.email:
        existing = (
            ...
            .filter(models.Patient.email == payload.email)
            .first()
        )
        if existing:
            raise HTTPException(
                status_code=409, detail="Email already used by another patient"
            )
...
@router.post(
    "/patients/{patient_id}/device", response_model=schemas.DeviceOut, status_code=201
)
def assign_device(
    patient_id: str,
    ...
    if patient.device:
        raise HTTPException(
            status_code=409, detail="Patient already has a device assigned"
        )

    device = (
        db.query(models.Device)
        .filter(models.Device.device_uid == payload.device_uid)
        .first()
    )
    if device and device.patient_id:
        raise HTTPException(
            status_code=409, detail="Device already assigned to a patient"
        )
...
def get_patient_device(
    ...
    if not patient.device:
        raise HTTPException(
            status_code=404, detail="No device assigned to this patient"
        )
    return patient.device


@router.get(
    "/patients/{patient_id}/device/secret", response_model=schemas.DeviceSecretOut
)
def reveal_device_secret(
    patient_id: str,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    db: Session = Depends(get_db),
):
    ...
    return schemas.DeviceSecretOut(
        device_uid=patient.device.device_uid, device_secret=patient.device.device_secret
    )


@router.post(
    "/patients/{patient_id}/device/regenerate-secret",
    response_model=schemas.DeviceSecretOut,
)
def regenerate_device_secret(
    patient_id: str,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    db: Session = Depends(get_db),
):
...
async def send_device_command(
    ...
    device = (
        db.query(models.Device).filter(models.Device.device_uid == device_uid).first()
    )
    ...
    command = models.DeviceCommand(
        device_id=device.id, command_type=command_type, payload=payload.payload
    )
    db.add(command)
    db.commit()
    db.refresh(command)
    ...
    command.status = (
        models.CommandStatus.SENT if delivered else models.CommandStatus.QUEUED
    )
    ...
    return {
        ...
        "note": None
        if delivered
        else "Device is offline; command is queued and will need to be resent, "
...
@router.get(
    "/patients/{patient_id}/medications", response_model=List[schemas.MedicationOut]
)
...
def _get_owned_medication(
    medication_id: str, caregiver: models.Caregiver, db: Session
) -> models.Medication:
    med = (
        db.query(models.Medication)
...
async def create_schedule(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="compartment must be one letter A-G"
        )
    try:
    ...
    if frequency == models.Frequency.SPECIFIC_DAYS and not payload.days_of_week:
        raise HTTPException(
            status_code=422,
            detail="days_of_week is required for specific_days frequency",
        )

    medications = (
        db.query(models.Medication)
        .filter(
            models.Medication.id.in_(payload.medication_ids),
    ...
    if len(medications) != len(set(payload.medication_ids)):
        raise HTTPException(
            status_code=422,
            detail="One or more medication_ids not found for this patient",
        )
    ...
    if existing:
        raise HTTPException(
            status_code=409,
            detail=f"Compartment {compartment.value} is already in active use",
        )
    ...
    _notify(
        db,
        patient,
        models.NotificationType.SCHEDULE_UPDATED,
        f"New schedule added for compartment {compartment.value} at {payload.dispense_time}",
        caregiver.id,
    )
...
@router.get(
    "/patients/{patient_id}/schedules", response_model=List[schemas.ScheduleOut]
)
def list_schedules(
    patient_id: str,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    ...
    require_owned_patient(patient_id, caregiver, db)
    schedules = (
        db.query(models.Schedule).filter(models.Schedule.patient_id == patient_id).all()
    )
    return [schedule_to_out(s) for s in schedules]


def _get_owned_schedule(
    schedule_id: str, caregiver: models.Caregiver, db: Session
) -> models.Schedule:
    schedule = (
...
async def update_schedule(
    ...
    if "medication_ids" in data:
        medications = (
            db.query(models.Medication)
            .filter(
                models.Medication.id.in_(data["medication_ids"]),
                models.Medication.patient_id == schedule.patient_id,
            )
            .all()
        )
...
async def force_sync_schedule(
    ...
    delivered = await manager.send_json(
        patient.device.device_uid, build_device_schedule_payload(db, patient)
    )
...
@router.get(
    "/patients/{patient_id}/dispense-logs",
    response_model=List[schemas.DispenseEventOut],
)
def list_dispense_logs(
    patient_id: str,
    limit: int = 100,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    db: Session = Depends(get_db),
...
@router.get(
    "/patients/{patient_id}/adherence-videos",
    response_model=List[schemas.AdherenceVideoOut],
)
def list_adherence_videos(
    patient_id: str,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    db: Session = Depends(get_db),
):
    ...
    videos = (
        db.query(models.Uploads)
        .filter(models.Uploads.type == schemas.UploadType.VIDEO)
        .order_by(models.Uploads.created_at.desc())
        .all()
    )
        ...
        formatted_videos.append(
            {
                ...
                "file_path": video_obj.url,
                "duration_seconds": 0,
                "person_detected": None,
                "uploaded_at": video_obj.created_at,
            }
        )
...
@router.get(
    "/patients/{patient_id}/telemetry", response_model=List[schemas.TelemetryOut]
)
def list_telemetry(
    patient_id: str,
    limit: int = 50,
...
@router.get(
    "/patients/{patient_id}/voice-interactions",
    response_model=List[schemas.VoiceInteractionOut],
)
def list_voice_interactions(
    patient_id: str,
    caregiver: models.Caregiver = Depends(get_current_caregiver),
    db: Session = Depends(get_db),
):
    ...
    audios = (
        db.query(models.Uploads)
        .filter(models.Uploads.type == schemas.UploadType.AUDIO)
        .order_by(models.Uploads.created_at.desc())
        .all()
    )
        ...
        formatted_audios.append(
            {
                ...
                "response_audio_path": audio_obj.url,
                "audio_format": "wav",
...
@router.get(
    "/patients/{patient_id}/notifications", response_model=List[schemas.NotificationOut]
)
...
def _notify(
    ...
    note = models.Notification(
        patient_id=patient.id, caregiver_id=caregiver_id, type=ntype, message=message
    )
    db.add(note)
    db.commit()
