...
// Buzzer is an active-LOW relay: LOW = energized/audible, HIGH = silent.
static inline void buzzerOn() { digitalWrite(PIN_BUZZER, LOW); }
static inline void buzzerOff() { digitalWrite(PIN_BUZZER, HIGH); }
...
void indicatorsTick() {
  ...
  switch (s_pattern) {
    ...
    case IndicatorPattern::PERSON_APPROACHED:
      // "changes the sound" once within range - faster clicking instead of a
      // tone change, since a relay can't vary pitch the way a piezo could
      digitalWrite(PIN_LED_GREEN, inWindow(elapsed, 400, 0, 200) ? HIGH : LOW);
      if (inWindow(elapsed, 400, 0, 100)) buzzerOn(); else buzzerOff();
      break;
    ...
    case IndicatorPattern::PICKUP_OK:
      // two quick clicks then settle - drawn from a short fixed sequence
      if (elapsed < 120) { buzzerOn(); digitalWrite(PIN_LED_GREEN, HIGH); }
      else if (elapsed < 220) { buzzerOff(); digitalWrite(PIN_LED_GREEN, LOW); }
      else if (elapsed < 340) { buzzerOn(); digitalWrite(PIN_LED_GREEN, HIGH); }
      else if (elapsed < 1200) { buzzerOff(); digitalWrite(PIN_LED_GREEN, LOW); }
